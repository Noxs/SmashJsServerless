const Event = require('../../../../lib/middleware/kinesisAnalytics/lib/event.js');
...
describe('Event', () => {
	...
	it('Test event instance success', () => {
		const rawEvent = { applicationArn: 'arn:aws:kinesisanalytics:xx-xxxx-x:xxxxxxxxxxxxx:xxxxxxxxxxxxxxx-ActionTest-env-one-region-x', records: [{ data: "eyJ0ZXN0IjoidGVzdCJ9" }] };
		const context = {};
	...
	it('Test event create failure', () => {
		const rawEvent = { applicationArn: 'arn:aws:kinesisanalytics:xx-xxxx-x:xxxxxxxxxxxxx:xxxxxxxxxxxxxxx-ActionTest-env-one-region-x', records: [{ data: "eyJ0ZXN0IjoidGVzdCI=" }] };
		const context = {};
		const terminate = {
			terminate: (error, data) => { },
		};
