...
function utils() {
    ...
    that.createNewRest = function (controllerName) {
        mkdirp.sync(path.resolve(path.join(rootPath, utilsDirectory)));
        that.copyFile(path.resolve(path.join(__dirname, templateDir, databaseFile)), path.resolve(path.join(rootPath, controllerDirectory, controllerName + ".js")));
        replace({
            regex: "CONTROLLER_NAME",
            replacement: controllerName,
            paths: [path.resolve(path.join(rootPath, controllerDirectory, controllerName + ".js"))],
            recursive: true,
            silent: true
        });
        console.log("Lambda create new controller " + controllerName + ".js");
