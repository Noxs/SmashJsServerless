...
class TooManyRequest extends SmashError {
...
class SmashErrorFactory {
	...
	createFromResponse(response) { //eslint-disable-line complexity
		...
		switch (response.statusCode) {
			...
			case 429:
				return this.tooManyRequestError(body.message, body.details, this.getRequestId(headers));
			case 500:
	...
	tooManyRequestError(externalMessage, details = null, internalMessage = null) {
		const CODE = 429;
		const error = this._createError(CODE, externalMessage ? externalMessage : MESSAGES[CODE]);
		error.internalMessage = internalMessage ? internalMessage : externalMessage;
		error.details = details;
	...
	isTooManyRequestError(error) {
		return error instanceof TooManyRequest;
	}
