...
class TooManyRequests extends SmashError {
...
class SmashErrorFactory {
	...
	createFromResponse(response) { //eslint-disable-line complexity
		...
		switch (response.statusCode) {
			...
			case 429:
				return this.tooManyRequestsError(body.message, body.details, this.getRequestId(headers));
			case 500:
				return this.internalServerError(body.message + " " + this.getRequestId(headers, ""));
			case 501:
				return this.notImplementedError(body.message + " " + this.getRequestId(headers, ""));
			case 503:
				return this.serviceUnavailableError(body.message + " " + this.getRequestId(headers, ""));
			default:
				return this.internalServerError(body.message + " " + this.getRequestId(headers, ""));
		}
	}
	...
	tooManyRequestsError(externalMessage, details = null, internalMessage = null) {
		const CODE = 429;
		const error = this._createError(CODE, externalMessage ? externalMessage : MESSAGES[CODE]);
		error.internalMessage = internalMessage ? internalMessage : externalMessage;
		error.details = details;
