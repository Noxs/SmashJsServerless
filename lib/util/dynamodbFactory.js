...
class DynamodbFactory {
    constructor(prefix, suffix, options) {
        ...
        if (typeof prefix !== 'string') {
            throw new Error("First parameter of DynamodbFactory(prefix, suffix, options) must be a string, " + this.logger.typeOf(prefix));
        }
        this._tablePrefix = prefix;
        if (typeof suffix !== 'string') {
            throw new Error("Second parameter of DynamodbFactory(prefix, suffix, options) must be a string, " + this.logger.typeOf(suffix));
        }
        this._tableSuffix = suffix;
        if (typeof options !== 'object') {
            throw new Error("Third parameter of DynamodbFactory(prefix, suffix, options) must be an object, " + this.logger.typeOf(options));
        }
    ...
    _buildDynamodb(config) {
        const databaseConfigBuilder = new DatabaseConfigBuilder(this._tablePrefix, this._tableSuffix, config);
        databaseConfigBuilder._translateDescription();
        const dynamodb = new Dynamodb(databaseConfigBuilder.transformedDescription);
        return dynamodb;
    }
