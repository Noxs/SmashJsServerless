...
class KinesisAnalyticsProcessor {
	...
	isEvent(event) {
		if (!event.applicationArn || !event.applicationArn.startsWith('arn:aws:kinesisanalytics') || !event.streamArn) {
			return false;
		}
		console.log("1) MATCHED EVENT PREPROCESSING", event);
		return true;
	}
	...
	expose() {
		return [
			{
				"functionName": "kinesisAnalyticsProcessor",
				"function": "event",
			},
		];
	}
...
module.exports = KinesisAnalyticsProcessor;
