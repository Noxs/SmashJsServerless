...
class WebSocket {
	...
	getUrl(connectionId) {
		return smash.getEnv("WEBSOCKET_API_ID") + ".execute-api." + smash.getRegion() + ".amazonaws.com" + this.getPath(connectionId);
	}

	postToConnection(params) {
		return new Promise((resolve, reject) => {
			const host = this.getUrl(params.connectionId);
			...
			request.post({ url, headers: signature.headers, json: true }, (error, response, body) => {
				if (error) {
