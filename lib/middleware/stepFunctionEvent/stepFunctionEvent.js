...
const logger = smash.logger("StepFunctionEvent");
const errorUtil = new smash.SmashError(logger);
const EventType = require("./lib/eventType.js.js");
const Event = require("./lib/event.js.js");

class StepFunctionEvent {
    constructor() {
        this._events = [];
    }
    ...
    handleEvent(rawEvent, context, callback) {
        ...
        if (matchedEvents.length === 0) {
            logger.info("No event found for: " + event.name);
            this.terminate(new Error("No event found for: " + event.name));
        } else if (matchedEvents.length > 1) {
            logger.error("More than 1 match for the event: " + event.name);
            this.terminate(new Error("More than 1 match for the event: " + event.name));
        } else {
            logger.info("Match event source: " + event.name + " in env: " + smash.getEnv("ENV"));
    ...
    isEvent(event) {
        if (!event.EventSource || !event.Route || event.EventSource != 'StateMachine') {
            return false;
        }
        return true;
    }
...
module.exports = StepFunctionEvent;
