...
const queryStringNative = require("querystring");
...
class Event {
	constructor(rawEvent, context, terminate) {
		...
		if (queryString.stringify(this._rawEvent.queryStringParameters) !== queryStringNative.stringify(this._rawEvent.queryStringParameters)) {
			logger.info("Query string diff");
		}
