...
class EventType {
	constructor(stream, callback) {
		...
		if (typeof stream.streamName !== 'string') {
			throw new Error("First parameter of EventType() must have a string property called streamName, " + errorUtil.typeOf(stream.streamName));
		}
		this._streamName = stream.streamName;
	...
	match(event) {
		return event.streamName === this.streamName;
	}
}
