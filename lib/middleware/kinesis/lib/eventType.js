...
class EventType {
	constructor(stream, callback) {
		...
		if (typeof stream.streamName !== 'string') {
			throw new Error("First parameter of EventType() must have a string property called queue, " + errorUtil.typeOf(stream.streamName));
		}
	...
	match(event) {
		if (event.streamName !== this._streamName) {
			return false;
		}
		return true;
	}
