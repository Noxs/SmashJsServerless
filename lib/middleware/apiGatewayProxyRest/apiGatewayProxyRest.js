...
const { RESPONSE_HEADERS_DEFAULT, CONTROLLER, SLASH } = require("./lib/constant");
...
class ApiGatewayProxyRest {
	...
	postRegisterHandlers() {
		this.router.registerMissingRoutes();
		return this;
	}
	...
	async handleRequest(request, response) {
		logger.info("Matched route: " + request.route.method + " " + request.route.path + " => " + request.path + "; version: " + request.route.version + "; original version: " + (request.route.originalVersion ? request.route.originalVersion : "none") + "; requested version: " + request.version + "; user: " + this.getPrintableUser(request));
		smash.setCurrentEvent(request);
		try {
			if (request.hasInRule()) {
				await request.cleanIn();
			} else if (smash.filter.hasInRule(request.route) === true) {
				logger.deprecated("Usage of inRule() is deprecated");
	...
	stack() {
		...
		while (stack[0].getFileName().indexOf(CONTROLLER) < 0) {
			stack.shift();
		}
	...
	buildTag() {
		const [root, paths] = path.dirname(this.lastControllerName()).split(CONTROLLER + path.sep);//eslint-disable-line no-unused-vars
		const tag = paths.split(path.sep);
		tag.shift();
		return tag.join(SLASH);
	}

	register(endpoint, callback) {
		const tag = this.buildTag();
		this.router.register({ tag, ...endpoint }, callback);
		return this;
	}
