...
class Response {
	...
	_changeErrorHeaders() {
		try {
			const accept = accepts(this.request);
			const contentType = accept.type(responseContentType.getErrorAvailableContentTypes());
			if (contentType === false) {
				throw errorUtil.notAcceptableError(this.request.headers.get(HEADERS.ACCEPT));
			} else if (contentType === DEFAULT_ACCEPT) {
				this.headers.set(HEADERS.CONTENT_TYPE, DEFAULT_CONTENT_TYPE);
			} else {
				this.headers.set(HEADERS.CONTENT_TYPE, contentType);
	...
	checkResponseCompliance(code) {
		if (code && this.request && this.request.isResponseCompliant) {
