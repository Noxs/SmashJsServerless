...
class Response {
	...
	cleanOk(body, context = {}) {
		...
		this._cleanAndAssign({ code: this.codes[200], body, context: { ...context, request: this.request, response: this } }).then(this.ok).catch(this.handleError);
	}
	...
	cleanCreated(body, context = {}) {
		this.checkResponseCompliance(this.codes[201]);
		this._cleanAndAssign({ code: this.codes[201], body, context: { ...context, request: this.request, response: this } }).then(this.created).catch(this.handleError);
	}
	...
	handleError(error) {
		try {
			...
			if (this.request && this.request.hasOutRule && this.request.hasOutRule(this.codes[error.code])) {
				...
				}).catch(this.handleEmergecyError);
			} else {
				this.handleResponse(error.code, error.body);
			}
		} catch (error) {
			this.handleEmergecyError(error);
		}
	}

	handleEmergecyError(parentError) {
