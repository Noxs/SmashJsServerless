	...
	_cleanAndAssign(body) {
		return new Promise((resolve, reject) => {
			if (smash.filter.hasOutRule(this.request.route) === true) {
				body = smash.filter.cleanOut(this.request.route, body);
				resolve(body);
			} else {
				reject(this.internalServerError("No outRule found for " + JSON.stringify(this.request.route)));
			}
		});
	}

	cleanOk(body) {
		this._cleanAndAssign(body).then(body => {
			this.ok(body);
		}).catch(error => {
