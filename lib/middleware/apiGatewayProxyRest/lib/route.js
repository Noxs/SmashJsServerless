...
class Route {
	...
	_validateRules(route) {
		if (route.request.parameters || route.request.body) {
			const inRuleProcessor = new InRuleProcessor();
			inRuleProcessor.validate({ parameters: route.request.parameters, body: route.request.body });
		}
		if (route.update) {
	...
	hasInRule() {
		return Boolean(this.request.parameters || this.request.body);
	}
	...
	cleanIn(request) {
		...
		return processor.use({ parameters: this.request.parameters, body: this.request.body }).to(request);
	}
