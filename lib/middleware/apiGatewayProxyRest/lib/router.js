...
class Router {
	...
	_registerRoutesBackward(routes, ordererdVersions) {
		for (let i = ordererdVersions.length - 1; i > 0; i--) {
			...
			routes[version].forEach(action => {
				if (routes[previousVersion].indexOf(action) === -1) {
					...
					const clonedRoute = { ...routeToRegister.originalRouteData, originalVersion: version, version: previousVersion };
					this.register(clonedRoute, routeToRegister.callback);
		...
		for (let i = 0; i < ordererdVersions.length - 1; i++) {
			...
			routes[version].forEach(action => {
				if (routes[nextVersion].indexOf(action) === -1) {
					const routeToRegister = this._findRoute({ action, version });
					const clonedRoute = { ...routeToRegister.originalRouteData, originalVersion: version, version: nextVersion };
					this.register(clonedRoute, routeToRegister.callback);
