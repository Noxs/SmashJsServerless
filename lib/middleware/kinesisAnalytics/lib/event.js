...
class Event {
	...
	_extractApplicationInfos(rawEvent) {
		...
		const applicationInfos = splittedApplicationArn.pop().split("-");
		this.project = applicationInfos.shift();
		this.application = applicationInfos.shift();
		this.env = applicationInfos.shift();
		this.region = applicationInfos.join("-");
		return this;
	}
