...
class EventType {
	constructor(kinesisAnalytics, callback) {
		...
		if (typeof kinesisAnalytics.application !== 'string') {
			throw new Error("First parameter of EventType() must have a string property called application, " + errorUtil.typeOf(kinesisAnalytics.application));
		}
		this._application = kinesisAnalytics.application;
		if (typeof kinesisAnalytics.stream !== 'string') {
			throw new Error("First parameter of EventType() must have a string property calledstream, " + errorUtil.typeOf(kinesisAnalytics.stream));
		}
		this._stream = kinesisAnalytics.stream;
	...
	match(event) {
		if (event.application !== this.application && event.stream !== this.stream) {
			return false;
		}
		return true;
	}
