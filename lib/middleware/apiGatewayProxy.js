...
class ApiGatewayProxy extends Next {
    constructor() {
        ...
        this._link();
        /* Not sure this is the good way for doing this */
        if (smash.config.get("user_repository.file")) {
            try {
                this._userProvider.attachRepository(require(path.resolve(path.join(process.cwd(), smash.config.get("user_repository.file")))));
            } catch (error) {
                this.error("Failed to attach user repository: " + smash.config.get("user_repository.file") + " to user provider", error);
                throw new Error("Failed to attach user repository: " + smash.config.get("user_repository.file") + " to user provider");
            }
        }
    }
    ...
    handleRequest(request, response) {
        ...
        this.info("Match route: " + request.route.method + " path: " + request.route.path + " version: " + request.route.version + " authorizations: " + (request.route.authorizations ? request.route.authorizations.join("||") : "None") + " for request: " + request.path + " in env: " + this.smash.getEnv("ENV"));
