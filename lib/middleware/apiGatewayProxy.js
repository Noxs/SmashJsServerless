...
class ApiGatewayProxy extends Next {
    ...
    _registerUserRepository() {
        if (this.smash.config.get("user_repository.file")) {
            const file = path.resolve(path.join(process.cwd(), this.smash.config.get("user_repository.file")));
            try {
                this._attachUserRepository(require(file));
            } catch (error) {
                this.error("Failed to load user repository from file: " + this.smash.config.get("user_repository.file") + " / " + file + " to user provider", error);
                throw new Error("Failed to load user repository from file: " + this.smash.config.get("user_repository.file") + " / " + file + " to user provider");
            }
        } else if (this.smash.config.get("user_repository.database")) {
            try {
                this._attachUserRepository(this.smash.database(this.smash.config.get("user_repository.database")));
            } catch (error) {
                this.error("Failed to load user repository from database: " + this.smash.config.get("user_repository.database") + " to user provider", error);
                throw new Error("Failed to load user repository from database: " + this.smash.config.get("user_repository.database") + " to user provider");
            }
        }
        return this;
    }
    ...
    _attachUserRepository(module) {
        ...
        try {
            ...
            this.error("Failed to attach user repository: " + this.typeOf(instance) + " given", error);
            throw new Error("Failed to attach user repository: " + this.typeOf(instance) + " given");
        }
        return this;
    }
    ...
    handleRequest(request, response) {
        ...
        this.info("Match route: " + request.route.method + " " + request.route.path + "; version: " + request.route.version + "; authorizations: " + (request.route.authorizations ? request.route.authorizations.join("||") : "None") + "; request: " + request.path + " in env: " + this.smash.getEnv("ENV"));
