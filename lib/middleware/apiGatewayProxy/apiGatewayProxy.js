...
const USER_REPOSITORY_BYPASS = "apiGatewayProxy.user_repository.bypass";
...
class ApiGatewayProxy extends Next {
    ...
    _registerUserRepository() {
        ...
        } else if (this.smash.config.get(USER_REPOSITORY_BYPASS)) {
            try {
                this._userProvider.bypassUserRepository(this.smash.config.get(USER_REPOSITORY_BYPASS));
            } catch (error) {
                this.error("Failed to bypass user repository", error);
                throw new Error("Failed to bypass user repository");
            }
        }
        return this;
    }
    ...
    handleRequest(request, response) {
        ...
        this.info("Match route: " + request.route.method + " " + request.route.path + "; version: " + request.route.version + "; authorizations: " + (request.route.authorizations ? request.route.authorizations.join(" || ") : "None") + "; request: " + request.path + " in env: " + this.smash.getEnv("ENV") + " with user: " + (request.user ? request.user.username : "Anonymous"));
