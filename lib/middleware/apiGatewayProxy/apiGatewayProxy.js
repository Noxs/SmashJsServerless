...
class ApiGatewayProxy extends Next {
    ...
    _registerUserRepository() {
        if (this.smash.config.get(USER_REPOSITORY_FILE)) {
            ...
            try {
                this._userProvider.bypassUserProvider(this.smash.config.get(USER_REPOSITORY_BYPASS));
            } catch (error) {
                this.error("Failed to bypass user provider", error);
                throw new Error("Failed to bypass user provider");
            }
        }
        return this;
    }
