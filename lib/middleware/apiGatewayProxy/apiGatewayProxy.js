...
class ApiGatewayProxy extends Next {
    ...
    handleEvent(event, context, callback) {
        ...
        if (typeof event !== "object" || this.isEvent(event) === false) {
            ...
            logger.error("Wrong type of event as argument to ApiGatewayProxy.handleEvent()");
    ...
    handleRequest(request, response) {
        if (request === undefined || request === null || request.constructor !== Request) {
            //FIX ME use smashError instead
            logger.error("First parameter of handleRequest() must be Request object type, " + this.typeOf(request));
            response.internalServerError("Internal server error");
            return this;
        }
        if (response === undefined || response === null || response.constructor !== Response) {
            //FIX ME use smashError instead
            logger.error("Second parameter of handleRequest() must be Response object type, " + this.typeOf(response));
            response.internalServerError("Internal server error");
            return this;
        }
        if (request.constructor !== Request || request.route.constructor !== Route) {
            //FIX ME use smashError instead
            logger.error("Missing matched route in request");
            response.internalServerError("Internal server error");
            return this;
        }
        logger.info("RequestId: " + request.requestId);
        logger.info("Matched route: " + request.route.method + " " + request.route.path + "; version: " + request.route.version + "; request: " + request.path + " in env: " + this.smash.getEnv("ENV") + " with user: " + (request.user ? request.user.username : "Anonymous"));
        ...
        if (smash.binder.requiredRuleExist(request.route.action) === true) {
            ...
            if (bodyIsValid !== true) {
                //FIX ME use smashError instead
                logger.info("Invalid body: ", bodyIsValid);
                response.badRequest("Invalid body", bodyIsValid);
                return this;
            }
        }
