...
const USER_PROVIDER_REPOSITORY_EXPOSE_PROPERTIES = "apiGatewayProxy.user_provider.repository.expose_properties";
...
class UserProvider extends Next {
    ...
    _loadConfig() {
        ...
        this._exposeProperties = smash.config.get(USER_PROVIDER_REPOSITORY_EXPOSE_PROPERTIES);
    }
            ...
            try {
                this._repository[this._method].apply(this._repository, this._getArguments(request.user)).then((user) => {
                    ...
                    if (user === null) {
                        ...
                        if (Array.isArray(this._exposeProperties) === true) {
                            for (let i = 0, length = this._exposeProperties.length; i < length; i++) {
                                if (user[this._exposeProperties[i]] !== undefined) {
                                    request.user[this._exposeProperties[i]] = user[this._exposeProperties[i]];
                                }
                            }
                        } else {
