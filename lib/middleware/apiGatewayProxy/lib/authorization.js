    ...
    handleRequest(request, response) {
        ...
        for (let i = 0, length = request.routes.length; i < length; i++) {
            if (request.routes[i].authorizations === null) {
                ...
                break;
            } else if (request.user && request.routes[i].hasRoles(request.user.roles) === true) {
                route = request.routes[i];
                break;
            }
        }
        if (route === null) {
            this.info("Not authorized " + (request.user ? (request.user.username ? request.user.username + " " + request.user.roles.join(" ") : "anonymous") : "anonymous") + " on path " + request.path + ", roles authorized " + route.authorizations.join(" "));
