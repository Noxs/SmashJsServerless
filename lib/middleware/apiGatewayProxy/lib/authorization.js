    ...
    handleRequest(request, response) {
        ...
        if (route === null) {
            if (request.user && request.user.username) {
                this.info("Forbidden " + request.user.username + " " + request.user.roles.join(" ") + " on path " + request.path);
                response.forbidden("Forbidden");
            } else {
                this.info("Not authorized anonymous on path " + request.path);
                response.notAuthorized("Not authorized");
            }
        } else {
            request.route = route;
