...
class Route extends smash.Console {
    constructor(method, route, callback) {
        ...
        if (typeof method !== 'string') {
            throw new Error("First parameter of Route() must be a string, " + this.typeOf(method));
        }
        if (METHODS[method] !== method) {
            throw new Error("First parameter of Route() can be GET, POST, PUT, PATCH, DELETE, OPTIONS, HEAD; " + this.typeOf(method));
        }
        this._method = method;
        if (typeof callback !== 'function') {
            throw new Error("Third parameter of Route() must be a function, " + this.typeOf(callback));
        }
        ...
        if (typeof route !== 'object') {
            throw new Error("Second parameter of Route() must be an object, " + this.typeOf(route));
        }
        if (typeof route.path !== 'string') {
            throw new Error("Second parameter of Route() must have a string property called path, " + this.typeOf(route.path));
        }
        this._path = route.path;
        if (typeof route.action !== 'string') {
            throw new Error("Second parameter of Route() must have a string property called action, " + this.typeOf(route.action));
        }
        this._action = route.action;
        if (route.version === undefined || route.version === null) {
            this._version = DEFAULT;
        } else if (typeof route.version !== 'string') {
            throw new Error("Second parameter of Route() can have (optionnal) a string property called version, " + this.typeOf(route.version));
        } else {
