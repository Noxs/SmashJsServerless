...
const logger = smash.logger("CodePipelineJobEvent");
const EventType = require("./lib/eventType.js");
const Event = require("./lib/event.js");

class CodePipelineJobEvent extends smash.Console {
    constructor() {
        super();
        this._smash = smash;
        this._events = [];
    ...
    handleEvent(rawEvent, context, callback) {
        ...
        if (matchedEvents.length === 0) {
            ...
            logger.info("CodePipelineJobEvent match event in env: " + this.smash.getEnv("ENV"));
    ...
    terminate(error, data) {
        if (error) {
            logger.info("CodePipelineJobEvent result: Failure");
        } else {
            logger.info("CodePipelineJobEvent result: Success");
        }
    ...
    event(callback) {
        const event = new EventType(callback);
