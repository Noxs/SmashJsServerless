const smash = require("../../../smash.js");
const EventType = require("./lib/eventType.js");
const Event = require("./lib/event.js");
const CODE_PIPELINE_JOB_EVENT = "codePipelineJobEvent";
const DOT = ".";
const ENV = "ENV";

class CodePipelineJobEvent extends smash.Console {
    constructor() {
        super();
        this._smash = smash;
        this._events = [];
    }

    get smash() {
        return this._smash;
    }

    _matchEvents(event) {
        const events = [];
        for (let i = 0, length = this._events.length; i < length; i++) {
            if (this._events[i].match(event) === true) {
                events.push(this._events[i]);
            }
        }
        return events;
    }

    _buildEnv() {
        const env = this.smash.config.get(CODE_PIPELINE_JOB_EVENT + DOT + ENV);
        if (env) {
            smash.setEnv(ENV, env);
        }
        return this;
    }

    handleEvent(rawEvent, context, callback) {
        if (typeof callback !== 'function') {
            throw new Error("Third parameter of handleEvent() must be a function, " + this.typeOf(callback) + " given");
        }
        this._callback = callback;
        this._buildEnv();
        const event = new Event(rawEvent, context, this);
        const matchedEvents = this._matchEvents(event);
        if (matchedEvents.length === 0) {
            this.info("No event found");
            this.terminate(new Error("No event found"));
        } else if (matchedEvents.length > 1) {
            this.error("More than 1 match for the event");
            this.terminate(new Error("More than 1 match for the event"));
        } else {
            this.info("CodePipelineJobEvent match event in env: " + this.smash.getEnv("ENV"));
            matchedEvents[0].callback.call(smash, event);
            return this;
        }
    }

    terminate(error, data) {
        this._callback(error, data);
        return this;
    }

    isEvent(event) {
        if (!event["CodePipeline.job"]) {
            return false;
        }
        return true;
    }

    event(callback) {
        const event = new EventType(callback);
        this._events.push(event);
        return this;
    }

    expose() {
        return [
            {
                "functionName": "codePipelineJobEvent",
                "function": "event"
            }
        ];
    }
}

module.exports = CodePipelineJobEvent;