...
class EventType {
    constructor(route=null, callback) {
        if(arguments.length > 2) {
            throw new Error("First parameter of EventType() must be an object, " + errorUtil.typeOf(arguments[0]));
        }
        ...
        if(arguments.length === 1) {
            if (typeof arguments[0] !== 'function') {
                throw new Error("Third parameter of EventType() must be a function, " + errorUtil.typeOf(arguments[0]));
            }
            if (arguments[0].length !== 1) {
                throw new Error("Third parameter of EventType() must be a function witch takes 1 parameter: Event, " + arguments[0].length + " given");
            }
