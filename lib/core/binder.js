...
class Binder {
    ...
    _registerRequiredRule(rule) {
        ...
        if (!(KEY.REQUIRED_LIST in rule)) {
            throw new MissingPropertyError(KEY.REQUIRED_LIST);
        }
    ...
    hasRequired(name, data) {
        ...
        const list = rule[KEY.REQUIRED_LIST];
        const errors = [];
        const inputArray = [];
        ...
        for (let i = 0, length = inputArray.length; i < length; i++) {
            ...
            for (let keyList in list) {
                if (!(list[keyList] in data)) {
                    errors.push({ type: ERRORS.MISSING, name: list[keyList] });
                } else {
                    //TODO
        ...
        for (let i = 0, length = newArray.length; i < length; i++) {
            ...
            for (let keySource in source) {
                if (rule.mode === MODE.RESTRICTIVE) {
                    ...
                    throw new UnknownModeError(rule.mode);//TO REMOVE ??  because not possible in theory
                }
            }
        }
