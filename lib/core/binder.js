...
const TYPES = {
    ...
    "unsigned integer": _.assign(defaultIntegerType, {
...
class Binder {
    ...
    mergeObject(name, newData, currentData = {}) {
        if (!(name in this._updateConfiguration))
            throw new UnknownRuleError(name);
        const dataToUpdate = this._getObjectDataByMode(name, newData);
        return _.assign({}, currentData, dataToUpdate);
    }
    ...
    update(name, newData, currentData) {
        ...
        return _.map(arrayNewData, (newData, i) => this.mergeObject(name, newData, arrayCurrentData[i]));
    }

    clean(name, collection) {
        if (!(name in this._cleanConfiguration))
            throw new UnknownRuleError(name);
        return _.map(_.concat([], collection), _.bind(this._getObjectDataByMode, this, name));
    }

    _getObjectDataByMode(name, object) {
