		...
		try {
			if (typeof data.current === "object" && data.current !== null && Array.isArray(data.current) === false) {
				...
				if (rule.current.transform) {
					const result = await rule.current.transform(data.current);
				...
				for (const key of keys) {
					...
					if (finded) {
						data.current = await processor.executeNext({ rule: { ...rule, current: finded }, data: { ...data, current: data.current } });// eslint-disable-line no-await-in-loop
					} else {
