	...
	return new Promise(async (resolve, reject) => {
		try {
			if (typeof data.current[rule.current.name] === 'object' && data.current[rule.current.name] !== null && Array.isArray(data.current[rule.current.name]) === false) {
				...
				await Promise.all(data.current[rule.current.name].mapToArray((value, key) => { // eslint-disable-line arrow-body-style
					return new Promise(async (resolve, reject) => {
						try {
							const finded = properties.find(property => property.name === key);
							if (finded) {
								data.current[rule.current.name] = await processor.executeNext({ rule: { ...rule, current: finded }, data: { ...data, current: data.current[rule.current.name] } });
							} else {
