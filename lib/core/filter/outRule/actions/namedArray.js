		...
		try {
			if (Array.isArray(data.current[rule.current.name])) {
				for (const key in data.current[rule.current.name]) {
					...
					data.current[rule.current.name][key] = await processor.executeNext({ rule: { ...rule, current: rule.current.content }, data: { ...data, current: value } });// eslint-disable-line no-await-in-loop
				}
