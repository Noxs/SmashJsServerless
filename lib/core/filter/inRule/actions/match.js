...
const { STRING, TYPE, CAST_TO, MATCH, DEFAULT, VALIDATE, MIN, MAX, ERRORS, OPTIONAL } = require("../../constant");
...
function execute({ rule, data }) {
	return new Promise((resolve, reject) => {
		...
		if (regexp.test(data.current.value)) {
			...
			reject(errorUtil.badRequestError("Invalid body", { type: ERRORS.MATCH, name: data.current.name, expected: rule.current.value, given: data.current.value }));
		}
	});
}
