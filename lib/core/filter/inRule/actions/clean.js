...
function execute({ rule, data }) {
	return new Promise(resolve => {
		...
		if (rule.current.value === true) {
			const properties = extend(getSibling(rule.parents, PROPERTIES));
			const names = extend(properties.map((userInput, key) => key));
			extend(data.current.value);
			data.current.value = data.current.value.pick(names);
		}
