...
const { DEFAULT, TRANSFORM, BODY, PARAMETERS, PROPERTIES, OPTIONAL, OBJECT, CLEAN, ERRORS, VALIDATE } = require("../../constant");
...
function execute({ rule, data }) {
	return new Promise((resolve, reject) => {
		...
		if (Array.isArray(data.current.value)) {
			reject(errorUtil.badRequestError("Invalid body", { type: ERRORS.TYPE, reason: ERRORS.REASON_TYPE, name: data.current.name, expected: OBJECT, given: data.current.value }));
