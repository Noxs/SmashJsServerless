...
const { FUNCTION, DEFAULT, TYPE, STRING, PROPERTIES, OPTIONAL, CAST_TO, MATCH, VALIDATE, MIN, MAX, NUMBER, ARRAY, OBJECT, BOOLEAN } = require("../../constant");
	...
	return new Promise(async (resolve, reject) => {
		...
		if (data.current.value === undefined) {
			if (isFunction(rule.current)) {
				try {
					data.current.value = await rule.current.value(rule.current);
				} catch (error) {
					return reject(error);
				}
			} else {
