	...
	return new Promise(async (resolve, reject) => {
		try {
			if (Array.isArray(source.current[rule.current.name])) {
				for (const key in source.current[rule.current.name]) {
					const value = source.current[rule.current.name][key];
					target.current[rule.current.name][key] = await processor.executeNext({ // eslint-disable-line no-await-in-loop
