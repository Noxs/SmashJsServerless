	...
	return new Promise(async (resolve, reject) => {
		try {
			if (typeof source.current[rule.current.name] === 'object' && Array.isArray(source.current[rule.current.name]) === false) {
				...
				if (!target.current[rule.current.name]) {
					target.current[rule.current.name] = {};
				}
				const properties = rule.current.properties;
				const keys = Object.keys(source.current[rule.current.name]);
				for (const key of keys) {
					const finded = properties.find(property => property.name === key);
					if (finded) {
						target.current[rule.current.name] = await processor.executeNext({ // eslint-disable-line no-await-in-loop
