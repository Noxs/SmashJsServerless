	...
	return new Promise(async (resolve, reject) => {
		try {
			if (rule.current.beforeMerge) {
				source.current[rule.current.name] = await rule.current.beforeMerge(source.current[rule.current.name]);
			}
			if (source.current[rule.current.name] || source.current[rule.current.name] === null) {
